...
def process_json_files(json_folder: str):
    ...
    for json_file in json_files:
        try:
            ...
            doc = Document(page_content=content, metadata={"source": data.get("name", "unknown"), "extension": data.get("extension", ""), "course": data.get("course", DEFAULT_COURSE), "file_type": "code", "original_size": data.get("st_size", 0)})
            chunks = text_splitter.split_documents([doc])
            all_chunks.extend(chunks)
        except Exception as e: print(f"Error processing {json_file.name}: {e}")
    return all_chunks
...
def process_pdfs(pdf_folder: str):
    ...
    for pdf_file in pdf_files:
        try:
            ...
            pages = loader.load()
            try: relative_path = pdf_file.relative_to(pdf_path); course = relative_path.parts[0] if len(relative_path.parts) > 1 else DEFAULT_COURSE
            except: course = DEFAULT_COURSE
            chunks = text_splitter.split_documents(pages)
            for chunk in chunks: chunk.metadata["course"] = course; chunk.metadata["file_type"] = "pdf"; chunk.metadata["source"] = pdf_file.stem
            all_chunks.extend(chunks)
        except Exception as e: print(f"✗ Error processing {pdf_file.name}: {e}")
    return all_chunks
...
def setup_rag_chain():
    ...
    if os.path.exists(CHROMA_DIR) and os.listdir(CHROMA_DIR):
        ...
        if not all_documents:
            raise ValueError("No documents were processed. Check your data folders and processing functions.")

        # Create and persist the vector store
        print(f"📦 Creating and persisting vector store with {len(all_documents)} documents...")
